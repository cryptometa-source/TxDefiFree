...
from TxDefi.UI.Components.AboutFrame import AboutFrame
from TxDefi.UI.Components.PnlFrame import PnlFrame
from TxDefi.UI.Components.CustomWindow import CustomWindow
from TxDefi.UI.Components.LogsFrame import LogsFrame
from TxDefi.UI.Components.StrategiesFrame import StrategiesFrame
...
class MainUi(threading.Thread):
    ...
    def __init__(self, txdefitk: TxDefiToolKit, is_muted = True, *args, **kwargs):
        ...
        #Set Env Editor
        self.env_editor = CustomWindow(self.root, "Env Editor", globals.configuration_logo_path)

        self.env_editor.geometry("600x800")    
        self.env_editor.center_window(600, 800)             
 
        env_editor_frame = EnvEditorUI(self.env_editor, txdefitk.APP_NAME, txdefitk.PROFILE_NAME)
        ...
        # Create the new window ahead of time and hide it initially
        self.strategies_window = CustomWindow(self.root, "Strategies Configurator", globals.configuration_logo_path)
        self.strategies_window.geometry("1450x915")
        self.strategies_window.center_window(1450, 915)

        self.strategies_frame = StrategiesFrame(self.strategies_window, txdefitk.trades_manager, bg_color=globals.darkBgColor, fg_color=globals.darkBgColor)
        self.strategies_frame.pack(fill="both", expand=True)

        self.pnl_window = CustomWindow(self.root, "PnL")
        self.pnl_frame = PnlFrame(self.pnl_window, bg_color=globals.darkBgColor, fg_color=globals.darkBgColor)
        self.pnl_frame.pack(fill="both", expand=True)

        self.about_window = CustomWindow(self.root, "About", globals.txdefi_jteye_path)
        about_frame = AboutFrame(self.about_window)
        about_frame.pack(fill="both", expand=True)
        
        self.about_window.geometry("400x300")
        self.about_window.center_window(400, 300)
    ...
    def _init_menu(self):
        ...
        self.about_label = ctk.CTkLabel(self.topFrame, text="About", cursor="hand2", text_color="grey")
        self.about_label.pack(side=ctk.LEFT, padx=5, pady=5)
        self.about_label.bind("<Button-1>", lambda e: self.about_window.show_window())

        config_button = gui_functions.create_icon_button(self.topFrame, globals.configuration_logo_path, self.env_editor.show_window)
        ...
        #TODO icons
        sol_button = gui_functions.create_icon_button(solStatusFrame, globals.sol_logo_path, lambda: print("Feature is in work"))
    ...
    def _init_bottom_row(self):
        ...
        checkbox = ctk.CTkCheckBox(strategies_settings_frame, text="Pause Feed", variable=self.pause_feed_checkbox_var, command=self._onclick_pause_feed)
        checkbox.pack(side=ctk.LEFT, fill=ctk.X, pady = 5)
              
        strategies_button = gui_functions.create_icon_button(strategies_settings_frame, globals.strategies_logo_path, self.strategies_window.show_window)
        strategies_button.pack(side=ctk.LEFT, fill=ctk.X, pady=1)
  ...
  #           widget.set_sol_label(str(solValue) + " SOL")   
    ...
    def _handle_ui_command(self, arg1: TradeCommand | LoadSideWidgetCommand):
        ...
        elif arg1.command_type == UI_Command.FAST_BUY:
            self.trades_manager.fast_buy(arg1.mint_address)         
        elif arg1.command_type == UI_Command.FAST_SELL:
            self.trades_manager.fast_sell(arg1.mint_address)        
    ...
    def _show_pnl_window(self, pnl: ProfitLoss):
        token_info = self.market_manager.get_token_info(pnl.token_address)

        if token_info:
            tx_uri = globals.default_solscanner_tx_uri +  "/" + pnl.tx_signature

            self.pnl_frame.set_image(tx_uri, token_info.metadata.symbol + "/SOL", pnl, self.market_manager.get_solana_price())
            self.pnl_window.show_window()
    ...
    def _process_pnl(self, arg1: ProfitLoss):
        ...
        if token_info:
            ...
            pnl_log = (f"PNL: {timestamp},{arg1.token_address},{pnl_percent.to_string(2)},{arg1.token_quantity.to_string(2)},{arg1.cost_basis.to_string(6)},{self.trades_manager.get_sol_balance().to_string(6)},"
                      f"{self.trades_manager.get_unrealized_sol().to_string(6)}")
            
            logger_util.logger.info(pnl_log)
       
            col0 = TextProperty(str(timestamp))
            col1 = ClickableProperty(lambda e: self._show_pnl_window(arg1), arg1.token_address)
    ...
    def close(self):
        for table in self.trade_tables.values():
            table.stop()

        self.strategies_frame.stop()
