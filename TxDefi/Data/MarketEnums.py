...
class TradeEventType(Enum):
    ...
    @staticmethod
    def to_enum(type_str: str)->Enum:
        if type_str.upper() == "TRANSFER":
            return TradeEventType.SOL_TRANSFER
        elif type_str.upper() == 'TRANSFERCHECKED':
            return TradeEventType.TOKEN_TRANSFER
        elif type_str.upper() == "BUY":
            return TradeEventType.BUY
        elif type_str.upper() == "SELL":
            return TradeEventType.SELL
        elif type_str.upper() == "EXCHANGE":
            return TradeEventType.EXCHANGE
        else:
...
class AlertReason(Enum):
    ...
    def to_string(reason: "AlertReason")->str: #TODO needs to be updated
        switcher = {
            0: "PROFIT_LIMIT",
            1: "LOSS_LIMIT",
            2: "DEV_JEETED",
