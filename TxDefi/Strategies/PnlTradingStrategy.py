...
class PnlTradingStrategy(AbstractTradingStrategy):
    ...
    def load_from_dict(self, strategy_settings: dict[str, any]):
        order = OrderWithLimitsStops.from_dict(strategy_settings)
    ...
    def process_event(self, id: int, event: any):
        if event == self.initial_order.token_address and self.event_process_lock.acquire(blocking=False):
            ...
            if sell_amount:
                ...
                real_token_balance = self.trades_manager.get_token_account_balance(self.initial_order.token_address, self.trades_manager.get_default_wallet_settings().get_default_signer().get_account_address())
                if real_token_balance.to_ui() <= 1: #low amount, need to stop this strategy
                    self.current_tokens_amount.value = 0    
                else:
                    if sell_amount.compare(self.current_tokens_amount) > 0 and real_token_balance.compare(self.current_tokens_amount) > 0:
                        sell_amount = self.current_tokens_amount
                    else:
                        sell_amount = real_token_balance

                    if sell_amount.value > 0:
    ...
    def get_status(self)->str:
        if not self.last_price:
            self.last_price = self.trades_manager.get_market_manager().get_price(self.initial_order.token_address)

        return f"Tokens left: {self.current_tokens_amount.to_ui()} Last Price: {self.last_price.to_ui()} SOL"
