...
from TxDefi.DataAccess.Decoders.PumpDataDecoder import PumpDataDecoder, BondingCurveData
...
class TokenInfoRetriever:
    ...
    def fetch_and_fill_inner_metadata(metadata: ExtendedMetadata, inner_uri: str)->bool:
        update_uri = False
        metadata.inner_metadata_uri = inner_uri

        if "cf-ipfs.com" in metadata.inner_metadata_uri: #This uri has been depricated
            metadata.inner_metadata_uri = TokenInfoRetriever.replace_outdated_uri(metadata.inner_metadata_uri, "cf-ipfs.com", "ipfs.io")
            update_uri = True

        inner_asset_data = http_utils.get_request(metadata.inner_metadata_uri, timeout=1)
        ...
        if inner_asset_data:              
            ...
            if update_uri:
                metadata.image_uri = TokenInfoRetriever.replace_outdated_uri(metadata.image_uri, "cf-ipfs.com", "ipfs.io")
            ...
            createdOn = inner_asset_data.get('createdOn', globals.default_screener_uri)
            ...
            if account_info and account_info.get('value') is not None:
                ...
                if asset_metadata:
                    ...
                    if has_token_supply_info:
                        ...
                        inner_metadata_uri = asset_metadata.get('uri')

                        if len(inner_metadata_uri) > 0:
                            self.fetch_and_fill_inner_metadata(ext_metadata, inner_metadata_uri)
    ...
    @staticmethod
    def replace_outdated_uri(full_uri: str, old_server_uri: str, new_server_uri)->str:       
        ret_full_uri = full_uri
        if old_server_uri in full_uri:
            ret_full_uri = ret_full_uri.replace(old_server_uri, new_server_uri)
        ...
        if len(token_info.metadata.sol_vault_address) > 0 and len(token_info.metadata.token_vault_address) > 0: 
            ...
            if account_info and account_info.get("value") is not None:
                ...
                if owner and owner == self.pump_decoder.program_address:  #Pump        
                    ...
                    if isinstance(instruction_data, BondingCurveData):
            ...
            for instruction in parsed_transaction.instructions:
                ...
                if isinstance(instruction.data, LiquidityPoolData):                   
                    ...
                    token_info = TokenInfo.create(program, token_address, sol_vault_address, token_vault_address, 
                                                  Amount.sol_scaled(0), Amount.tokens_ui(0, 6))
