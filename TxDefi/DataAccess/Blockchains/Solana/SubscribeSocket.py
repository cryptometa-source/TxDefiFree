...
class SubscribeSocket(MarketDataSocket):    
    ...
    def process_data(self, data: str):   
        ...
        if json_data:
            ...
            elif not decoded_data and 'error' in json_data:
                message = json_data['error'].get('message', '')
                print(f"Can't process data from {self.wss_uri} {json_data}: {message} {self.out_topic}")
                #raise Exception(f"Can't process data from {self.wss_uri} {json_data}: {message}")
